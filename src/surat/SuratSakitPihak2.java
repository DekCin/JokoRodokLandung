        ...
        if(tabMode.getRowCount()==0){
            ...
            if(TCari.getText().trim().equals("")){
                Valid.MyReportqry("rptDataSuratSakitPihak2.jasper","report","::[ Data Surat Keterangan Sakit Pihak Ke 2 ]::",
                     ...
                     "suratsakitpihak2.umur,suratsakitpihak2.jk,suratsakitpihak2.alamat,suratsakitpihak2.hubungan,suratsakitpihak2.pekerjaan,suratsakitpihak2.instansi,dokter.nm_dokter"+
                     "from suratsakitpihak2 inner join reg_periksa on suratsakitpihak2.no_rawat=reg_periksa.no_rawat "+
                     "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "+
                     "left join dpjp_ranap on dpjp_ranap.no_rawat=suratsakitpihak2.no_rawat "+
                ...
                Valid.MyReportqry("rptDataSuratSakitPihak2.jasper","report","::[ Data Surat Keterangan Sakit Pihak Ke 2 ]::",
                     ...
                     "suratsakitpihak2.nama2 like '%"+TCari.getText().trim()+"%' or suratsakitpihak2.hubungan like '%"+TCari.getText().trim()+"%' or "+
    ...
    private void MnCetakSuratSakit2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnCetakSuratSakit2ActionPerformed
       if(TPasien.getText().trim().equals("")){
        ...
        this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                ...
                // Ambil kd_dokter dari dpjp_ranap
            ...
            // Ambil hash sidik jari dokter
            finger = Sequel.cariIsi("select sha1(sidikjari.sidikjari) from sidikjari " +
                "inner join pegawai on pegawai.id=sidikjari.id where pegawai.nik=?", kodedokter);

            // Format tanda tangan digital untuk laporan
            param.put("finger", 
                ...
                Sequel.cariIsi("select DATE_FORMAT(reg_periksa.tgl_registrasi,'%d-%m-%Y') from reg_periksa where no_rawat=?", TNoRw.getText()));
                Valid.MyReportqry("rptSuratSakitPihakKe2.jasper","report","::[ Surat Sakit Pihak Ke 2 ]::",
                        ...
                        " from suratsakitpihak2 inner join perusahaan_pasien inner join reg_periksa inner join pasien inner join dpjp_ranap inner join dokter inner join kelurahan inner join kecamatan inner join kabupaten" +
                        " on pasien.perusahaan_pasien=perusahaan_pasien.kode_perusahaan and reg_periksa.no_rkm_medis=pasien.no_rkm_medis and dpjp_ranap.no_rawat=reg_periksa.no_rawat and dpjp_ranap.kd_dokter=dokter.kd_dokter and pasien.kd_kel=kelurahan.kd_kel "+
